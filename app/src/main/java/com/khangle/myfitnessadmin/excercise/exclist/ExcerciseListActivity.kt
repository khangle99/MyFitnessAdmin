...
class ExcerciseListActivity : ComposableBaseActivity() {
    ...
    lateinit var categoryNameTV: TextView
    ...
    private fun validateInput(): Boolean { // false khi fail
        if (categoryNameTV.getText().toString().trim { it <= ' ' }.length == 0) {
            categoryNameTV.setError("Không được để trống tên")
            return false
        }
        if (pickedUri == null) {
